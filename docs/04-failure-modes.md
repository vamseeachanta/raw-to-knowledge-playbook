...
| `formula-loss` | Spreadsheet **calc-logic lost** — only computed values captured (`data_only`) | xlsx text/CSV lane reads values, not formulas | Formula-density probe vs extract | **Validated:** second `data_only=False` load emits a per-sheet formulas CSV only where formulas exist; values CSV doubles as the test oracle for any later formula→code conversion. Chart series still a richer lane |
...
| `attachment-drop` | Email **attachments dropped**; only the body text ingested | msg lane writes header + body only | Independent attachment census (name+sha256+size) vs the extract | **Validated — inventory-then-recurse (GP-40):** every attachment inventoried in the parent extract; format-lane attachments recursed through the same extractors; images inventory-only; attachment bytes temp-dir only |
| `derived-name-collision` | Two attachments' **derived outputs collide** on a slugged filename → silent overwrite (the inventory still lists both, masking the loss) | Output name derived from filename stem alone; single-rewrite dedup heuristics (an ordinal-suffixed name can collide with a later *real* stem) | Adversarial reproducer (synthetic same-stem attachments through the lane) | Uniqueness **by construction**: candidate loop exits only on an unused name (GP-41); never guard with fail-if-exists in idempotent re-run pipelines |
