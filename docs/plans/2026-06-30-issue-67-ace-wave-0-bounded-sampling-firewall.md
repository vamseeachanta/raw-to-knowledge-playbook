...
> **Project:** N/A
> **Lane:** lane:codex
> **Review artifacts:** pending formal provider review
...
- Source: `docs/plans/ace-share-ingestion-wave-coordination.md`
  - Finding: The structural wave registry marks [#52](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/52)-[#60](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/60) as manifest-backed ingestion waves and [#62](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/62) as the manifest freshness gate. [#67](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/67) will require recorded [#62](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/62) snapshot evidence before downstream manifest-backed sampling, but it will not implement [#62](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/62)'s freshness validator.
...
- [#69](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/69) owns the repo-local legal/security scan gate. [#67](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/67) closeout will record `NO_LEGAL_SCAN_SCRIPT` if that gate remains unavailable.
...
- Allowed manifest source names will be the public metadata keys already named by the portfolio gate: `INDEX.md`, `assets.json`, `docs/master-index.jsonl`, `_cad-index/index-summary.json`, `_cad-index/cad-readability-index.tsv`, and `.ace-knowledge/index.db`.
...
- No standalone [#67](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/67) plan exists yet.
...
| `required_sampling_fields` | manifest source, seed, sort rule, per-bucket row cap, maximum files touched, maximum bytes touched, request class, [#62](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/62) evidence requirement, and output shape |
...
| `request_classes` | `control_plane_proof`, `downstream_manifest_backed_sampling`, and `metadata_only_fixture` |
...
validate allowed manifest source enum:
  exact public metadata key set from portfolio gate
validate bounded sampling grammar:
  request class is closed
  manifest source is present and allowed
  seed is present and deterministic
  sort rule is present and references schema-declared row-key semantics without exposing raw values
...
validate executable contexts:
  markdown, workflow, Python, and JSON contexts are classified by closed rules
...
| Modify | `docs/plans/README.md` | Add this plan row and update the wave-0 split registry to draft status without implying review approval |
| Modify | `docs/plans/ace-share-ingestion-wave-coordination.md` | Point the [#67](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/67) split row at this plan and keep implementation readiness false |
...
| `test_bounded_sampling_fields_are_required` | Sampling request grammar is complete | Synthetic request missing one field at a time | Missing manifest source, seed, sort rule, per-bucket cap, max files, max bytes, request class, or output shape fails |
...
- [ ] Bounded sampling requests require manifest source, deterministic seed, sort rule, per-bucket row cap, max files touched, max bytes touched, request class, and metadata-only output shape.
...
- [ ] `UV_CACHE_DIR=.claude/state/uv-cache uv run python scripts/validate_ace_bounded_sampling_firewall.py` passes after implementation.
- [ ] `UV_CACHE_DIR=.claude/state/uv-cache uv run python -m unittest tests.test_validate_ace_bounded_sampling_firewall` passes after implementation.
- [ ] `UV_CACHE_DIR=.claude/state/uv-cache uv run python scripts/validate_ace_wave0_schema_contract.py` still passes after implementation.
- [ ] `UV_CACHE_DIR=.claude/state/uv-cache uv run python scripts/validate_ace_epic_wave_coordination.py` still passes after implementation.
- [ ] `UV_CACHE_DIR=.claude/state/uv-cache uv run skills/validate_skill.py` still passes after implementation.
...
- [ ] If `scripts/legal/legal-sanity-scan.sh` is still unavailable at closeout, the issue comment records `NO_LEGAL_SCAN_SCRIPT` and points to [#69](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/69).
...
|---|---|---|
| Claude | PENDING | Formal provider review has not run. |
| Codex | PENDING | Formal provider review has not run. |
| Gemini | PENDING | Formal provider review has not run. |
