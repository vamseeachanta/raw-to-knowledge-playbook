...
define legal/security closeout gate:
  ...
  the deny-list schema is YAML parsed by the bash wrapper without yq: version,
  default_severity, exclusions, and one or more pattern groups; each pattern entry carries
  pattern, case_sensitive, description, and optional severity defaulting to default_severity
  the initial deny-list must include block-severity synthetic/generic patterns for:
    private host/source path roots, including the private mount root literal
...
| test_legal_sanity_scan_is_repo_local_and_executable | Inherited legal/security gate is concrete | `.legal-deny-list.yaml`, `scripts/legal/legal-sanity-scan.sh`, and changed-file fixtures | Repo-local command `bash scripts/legal/legal-sanity-scan.sh --diff-only` exists, loads the documented repo-local deny-list schema, scans changed/staged files, passes clean fixtures, and fails block-severity fixtures for the minimum pattern classes named in the legal/security closeout gate |
