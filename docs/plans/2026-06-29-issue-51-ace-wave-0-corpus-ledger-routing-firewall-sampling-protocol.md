...
- Existing governance docs and sibling plans include mixed `sha256` references. #51 must classify every unique `path:line` hit surfaced by `rg -n "sha256|source hash|provenance pointer" docs --glob "*.md"`, modify only claims that describe raw source hashes/sha256 pointers as always safe, public-safe, public provenance, or public source references, and record no-change rationale for private-sidecar, LFS/OID, census, schema, validator, or private-ledger contexts. After the sweep, raw `source_sha256` values are private-sidecar provenance while public artifacts reference only opaque `public_source_token` values.
...
- `scripts/ace_public_contract.py` will own JSON contract loading, token-generation helpers, private/public field classifications, and source-hash sweep classification helpers.
...
define ledger required fields:
  ...
  share_relative_path_private_only, downstream_issue, extension_family, content_class, sensitivity,
...
define sampling command grammar:
  ...
  that do not contain executable prefixes, source paths, pipes, redirects, or command flags
...
scan #51 public surfaces before commit/comment:
  artifacts/ace-share-wave0-control-plane.md, config/ace-public-surface-deny-list.json,
  config/ace-public-output-contract.json, .github/workflows/validate.yml,
  scripts/ace_public_tokens.py, scripts/validate_ace_wave0_control_plane.py,
...
define docs publication boundary:
  ...
  published; this plan, the #63 plan, README, coordination docs, and retained review
  artifacts must pass #51 generic self-scan before commit/posting
...
define public-surface scanner field-name policy:
  ...
  coordination docs, artifacts/ace-share-wave0-control-plane.md, skill instructions,
  validator constants, and tests
...
reconcile existing governance docs:
  run `rg -n "sha256|source hash|provenance pointer" docs --glob "*.md"`
  ...
  classify each hit in artifacts/ace-source-hash-policy-sweep.md as:
    ...
    always safe, public-safe, public provenance, or a public source reference
...
define #51 generic self-scan boundary:
  ...
  creates/touches; it does not load private deny-lists, does not certify real public
  exposure, and does not duplicate the #63 public-output scanner
...
define review artifact evidence:
  ...
  review evidence for status:plan-review or approval readiness
define review artifact normalization:
  ...
  only path/identifier token substitutions applied
...
| Create | artifacts/ace-source-hash-policy-sweep.md | Public-safe classification report for every `sha256`/source-hash/provenance-pointer hit, with modify/no-change rationale |
...
| Create | scripts/ace_public_contract.py | JSON contract loader and token/hash-policy helper module |
...
| Modify | docs/plans/2026-06-29-issue-63-ace-public-output-redaction-and-identifier-canary.md | Reconcile the draft #63 public-output canary plan so `config/ace-public-output-contract.json` is the authority and local lists are examples only |
...
|---|---|---|---|
| test_ace_ledger_schema_requires_all_control_fields | Ledger schema covers #51 fields | Control-plane ledger table | Source token/hash fields, private lookup key, downstream issue, eval data, route, target store, extraction level, lifecycle, verification state, success applicability, public clearance evidence, independent reviewer/review artifact fields, expected yield, measured success fields, threshold, and validation command present |
...
| test_public_token_generator_has_no_source_inputs | #51 fixtures do not hand-author concrete public tokens | `scripts/ace_public_tokens.py` generator signature and good fixtures | Token generator takes no source path/name/hash/key parameters, uses `secrets.token_hex(16)`, emits `pst_[0-9a-f]{32}`, and the validator rejects any committed concrete `public_source_token` value in good fixtures instead of expanding a generation request marker at validation time |
...
| test_existing_governance_docs_do_not_publish_raw_hashes | Existing docs/plans cannot contradict #51 token policy | Unique `path:line` hits from `rg -n "sha256|source hash|provenance pointer" docs --glob "*.md"` plus `artifacts/ace-source-hash-policy-sweep.md` | Every hit is classified as `modify_public_safe_hash_claim` or `no_change_private_context`; modified claims no longer say raw hash pointers are always/public safe; no-change hits have an allowed private/LFS/census/schema/validator rationale |
...
| test_manifest_snapshot_gate_matrix_covers_all_ingestion_waves | Future ingestion waves cannot escape #62 gate silently | Child issue map | The `requires_manifest_snapshot_id=true` set exactly equals all ingestion wave rows (#52-#60 at this wave definition) and must be updated if a new ingestion wave is added |
...
| test_public_surface_self_scan_blocks_raw_identifiers | #51 public-surface safety | Control-plane artifact, token generator, validator, deny-list config, public-output contract config, workflow, good fixtures, plan, #63 plan, README, coordination doc, parent coordination validator/tests, touched skills, review artifacts, sidecars, and operator-fetched issue body/comment snapshots | Blocks raw paths, generic private-like identifier patterns, personal identifiers, proprietary snippets, and private lookup maps before commit/comment/status transition; real client/project/internal-name deny-list coverage remains a #63 gate |
...
| test_review_artifact_normalization_preserves_verdict | Public review artifacts can be redacted without changing verdict evidence | Raw/local and normalized review artifact fixtures | Only path/identifier token substitutions are allowed; authoritative verdict before and after normalization must match; verdict value is never hand-edited |
...
- [ ] Every unique `path:line` hit from `rg -n "sha256|source hash|provenance pointer" docs --glob "*.md"` is recorded in `artifacts/ace-source-hash-policy-sweep.md` with one of two outcomes: `modify_public_safe_hash_claim` for claims that raw source hashes/sha256 pointers are always safe, public-safe, public provenance, or public source references; or `no_change_private_context` for LFS/OID examples, private-sidecar provenance, attachment/manifest census fields, schema-field-name discussion, validator metadata, and sample private ledger rows. Every `modify_public_safe_hash_claim` hit is edited so raw `source_sha256` values are private-sidecar provenance and are not described as public-safe source references.
...
- [ ] The closed snapshot gate matrix records [#51](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/51), [#61](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/61), [#62](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/62), and [#63](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/63) as non-sampling with `requires_manifest_snapshot_id=false`; requires [#52](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/52)-[#60](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/60) to carry `requires_manifest_snapshot_id=true`; asserts that this true set exactly equals all current ingestion wave rows; and rejects any row that omits or falsifies the expected field.
...
- Before plan-review, `gh issue view 51 --json body --jq .body > <tmp-issue-body-file>` followed by `UV_CACHE_DIR=.claude/state/uv-cache uv run python scripts/validate_ace_epic_wave_coordination.py --scan-public-path <tmp-issue-body-file>` passes for the sanitized issue body, because `scripts/validate_ace_wave0_control_plane.py` will not exist until approved #51 implementation.
- Any #51 plan-review comment body is checked with `UV_CACHE_DIR=.claude/state/uv-cache uv run python scripts/validate_ace_epic_wave_coordination.py --scan-public-path <tmp-comment-body-file>` before posting. After #51 is approved and implemented, implementation closeout comments are checked with the #51 validator's `--scan-public-path` mode. Stronger private-deny-list/publication certification remains deferred to [#63](https://github.com/vamseeachanta/raw-to-knowledge-playbook/issues/63).
...
**Overall result:** r15 returned MAJOR from both active providers. This plan remains draft-only and must not move to `status:plan-review`. The next revision must resolve the `source_sha256` governance decision explicitly, define degraded quorum handling for Gemini unavailability, add all decomposed modules to self-scan targets, include public skill files in hash-policy classification, and clarify token helper ownership before another review round.
...
- **Risk:** Gemini has been unavailable in noninteractive review runs; final review evidence must disclose any degraded provider quorum and cannot count `UNAVAILABLE` artifacts as provider approval.
