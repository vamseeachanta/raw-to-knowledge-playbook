...
**Overall result:** r22 returned MAJOR from both active providers and Gemini UNAVAILABLE due noninteractive auth. This plan remains draft-only and must not move to `status:plan-review`. The current patch is intended to address the r22 actionable blockers by broadening the self-scan target to all sweep-modified public docs/plans/skills, removing the #51-CI-vs-#63-plan-prose contradiction, hardening the parent fallback scanner for private-field table rows and unlisted metadata evidence rows, and making source-inventory prose self-scan-safe. After those repairs are committed, a fresh review round must return no active-provider MAJORs before any plan-review label or user approval request.
