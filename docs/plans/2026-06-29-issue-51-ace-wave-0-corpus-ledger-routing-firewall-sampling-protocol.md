...
define public-surface scanner field-name policy:
  ...
  artifacts/ace-source-hash-policy-sweep.md, skill instructions, validator constants, and tests
...
define #51 generic self-scan boundary:
  ...
  validator files, retained review artifacts, same-stem review sidecars present at
...
**Overall result:** r15 returned MAJOR from both active providers. This plan remains draft-only and must not move to `status:plan-review`. This draft has since been patched for the ACE source-hash membership-inference governance decision, deterministic dual-purpose hash-hit precedence, assigned-digest removal, hash-policy sweep self-scan coverage, decomposed module self-scan targets, public skill hash-policy classification, pre-implementation parent-scanner fallback with traversal denial, exact comment body scan/post/refetch sequencing, reviewed-draft identity binding, AGENTS-consistent degraded quorum handling, and exclusive token-generator ownership. A fresh post-patch review round must return no-MAJOR before any status transition.
