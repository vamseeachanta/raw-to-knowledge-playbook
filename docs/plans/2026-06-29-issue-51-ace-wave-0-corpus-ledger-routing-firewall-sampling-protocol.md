...
**Overall result:** r18 returned Claude MINOR, Codex MAJOR, and Gemini UNAVAILABLE due noninteractive auth. This plan remains draft-only and must not move to `status:plan-review`. This draft has since been patched for r18 findings: structural token-marker self-scan scoping, bounded token collision retry wording, registered-row snapshot-gate scope, JSON-only config terminology, live #63 issue-body reconciliation, `skills/**/*.md` hash-sweep coverage, committed review-artifact role tagging, and denied-token prose cleanup. A fresh post-patch review round must return no-MAJOR before any status transition.
