...
- **(MINOR/cosmetic) The authoritative sweep string carries a redundant glob (lines 252, 341, 496, 541).** In `rg --glob "*.md" --glob "SKILL.md"`, inclusive globs OR together and `SKILL.md is a subset of *.md`, so the second glob is a no-op. Harmless, but the plan elevates this exact string to a frozen, scanner-allowlisted "exact shape," baking the redundancy into the contract.
